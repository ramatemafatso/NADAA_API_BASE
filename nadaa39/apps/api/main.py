...
async def _provider_openai_compatible(base_url: str, key: str, model: str, message: str, context: dict[str, Any], extra_headers: dict[str, str] | None = None) -> str:
    payload = {
        "model": model,
    ...
    if not content:
        raise RuntimeError(f"Provider returned no text: {data}")
    return str(content)


async def _provider_huggingface(message: str, context: dict[str, Any]) -> str:
    return await _provider_openai_compatible(
        "https://router.huggingface.co/v1/chat/completions",
        os.environ["HF_TOKEN"],
        os.getenv("HF_MODEL", "openai/gpt-oss-120b:fastest"),
        message,
        context,
    )
...
async def mpi(message: str, context: dict[str, Any]) -> dict[str, Any]:
    ...
    # Keep the existing providers and prefer the fastest/free-first routes before paid-capable APIs.
    if os.getenv("GEMINI_API_KEY"):
        providers.append(("gemini", _provider_gemini))
    if os.getenv("GROQ_API_KEY"):
        providers.append(("groq", lambda m, c: _provider_openai_compatible("https://api.groq.com/openai/v1/chat/completions", os.environ["GROQ_API_KEY"], os.getenv("GROQ_MODEL", "openai/gpt-oss-120b"), m, c)))
    if os.getenv("OPENROUTER_API_KEY"):
        providers.append(("openrouter", lambda m, c: _provider_openai_compatible("https://openrouter.ai/api/v1/chat/completions", os.environ["OPENROUTER_API_KEY"], os.getenv("OPENROUTER_MODEL", "openrouter/free"), m, c, {"HTTP-Referer": os.getenv("APP_PUBLIC_URL", "http://localhost:5173"), "X-Title": "Nadaa"})))
    if os.getenv("DEEPSEEK_API_KEY"):
        providers.append(("deepseek", lambda m, c: _provider_openai_compatible("https://api.deepseek.com/chat/completions", os.environ["DEEPSEEK_API_KEY"], os.getenv("DEEPSEEK_MODEL", "deepseek-v4-flash"), m, c)))
    if os.getenv("XAI_API_KEY"):
        providers.append(("grok", lambda m, c: _provider_openai_compatible("https://api.x.ai/v1/chat/completions", os.environ["XAI_API_KEY"], os.getenv("XAI_MODEL", "grok-4.6"), m, c)))
    if os.getenv("HF_TOKEN"):
        providers.append(("huggingface", _provider_huggingface))
    preferred = str(context.get("provider", "")).strip().lower() if context else ""
    if preferred:
...
async def providers_status():
    ...
    return {"free_first": True, "providers": configured}
