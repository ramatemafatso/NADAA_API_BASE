...
Nadaa 39 is a free-first, cross-platform personal AI assistant app architecture. It combines a multimodal voice interface, model routing, local speaker verification, web/file ingestion, document generation, and device automation behind one assistant identity.
...
- MPI router with Gemini + OpenAI-compatible providers (Groq/OpenRouter/DeepSeek/xAI Grok/Hugging Face) and automatic fallback.
...
- Native-app shell using Capacitor for Android/iOS and Tauri for desktop.
- GitHub Actions workflows for Android, iOS, and desktop app builds.
- No GitHub Pages / public website deployment workflow is included.
...
## Production / GitHub Pages / phone installs

The frontend is safe to publish because provider secrets stay on the FastAPI gateway. Do **not** put `GEMINI_API_KEY`, `DEEPSEEK_API_KEY`, `XAI_API_KEY`, `GROQ_API_KEY`, `OPENROUTER_API_KEY`, or `HF_TOKEN` in `apps/web/.env*` or GitHub Pages variables.

For GitHub Pages, deploy the FastAPI service separately on an HTTPS host, then create a GitHub Actions repository variable named `NADAA_API_BASE` containing that gateway origin, for example `https://api.example.com`. The included Pages workflow injects it at build time.

The web build uses relative asset paths, so it works on a repository/project Pages URL as well as a custom domain. The Capacitor workflows generate Android/iOS native assets from `resources/icon.svg`, so installed builds use the Morbius-strip Nadaa icon rather than a generic placeholder.

GitHub Pages itself cannot run the Python/FastAPI backend; Pages is static hosting. The API gateway is therefore intentionally separated from the public frontend.
...
cd apps/web
npm install
npm run dev  # local UI development only; ship through Capacitor/Tauri
```
